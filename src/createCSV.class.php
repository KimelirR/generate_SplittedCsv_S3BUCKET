...
class Csv{
    ...
    public function createCSV($part,$mydir,$rows,$path)
     {
       echo("[\033[0;37m".current_time()."\033[1;37m] Generating..\n");
       $json_lines = (new JsonLines())->delineEachLineFromFile("${mydir}/${part}");
                    ...
                    $csvFile = "${folder_name}/${part}.csv";
                    $handle = fopen($csvFile, "w");
                    if ($handle === false) { exit("Error creating $csvFile"); }
                        // CREATE CSV HEADERS
                    $headers = array('Id', 'Total_Funding', 'Title', 'Team_members', 'Type', 
                    'Salary', 'Salary_low', 'Salary_high', 'Salary_currency', 'Salary_time_unit', 'Career_Website', 'Job_Status', 'Location', 
                    ...
                    foreach ((object)$json_lines as $json_line){
                        ...
                        $description =clean_text($attributes['description']);
                                    ...
                                    $status = '';
                                    if($is_job_closed == false){ $status = 'active'; }
                                    else{ $status = 'closed'; }
                                    
                                        // Total funding
                                    $totalFunding = funding($company_domain,$rows);
                                    $fundingtxt = '';
                                    if($totalFunding>5000000){$fundingtxt = clean_text('> 5 million $');}
                                    elseif($totalFunding>1000000 && $totalFunding <= 5000000){ $fundingtxt = clean_text('1-5 million $');}
                                    elseif($totalFunding<1000000){$fundingtxt = clean_text('< 1 million $');}
                                    else{$fundingtxt = clean_text('Bootstrapped');}
                            
                                    //Team Members
                                    $teamMembers = employee_range($company_domain,$rows);
                                    $membernotext ='';
                                    if($teamMembers === '0-10'){$membernotext = clean_text('0-10');}
                                    elseif($teamMembers === '11-50'){$membernotext = clean_text('11-50');}
                                    elseif($teamMembers === '51-100'){$membernotext = clean_text('51-100');}
                                    elseif($teamMembers === '101-250' || $teamMembers === '251-500'){$membernotext = clean_text('101-500');}
                                    elseif($teamMembers === '501-1000'){$membernotext = clean_text('501-1000');}
                                    elseif($teamMembers === '1001-5000'){$membernotext = clean_text('1001-5000');}
                                    else{$membernotext = clean_text('>5000');}
